...
def main():
    ...
    for mask in config['masking']:
        ...
        if heterogeneous_exp:
            ...
            elif 'Film' in config['model_type'] or 'SEHNN' in config['model_type']:
        ...
        for location in locations:
            model_args['filmblock_args']['location'] = location

            for activation in ['sigmoid', 'tanh', 'linear']:
                model_args['filmblock_args']['activation'] = activation

                for scale, shift in [(True,True), (True,False), (False,True)]:
                    model_args['filmblock_args']['scale'] = scale
                    model_args['filmblock_args']['shift'] = shift
                                ...
                                for weight_decay in config['weight_decay']:
                                    ...
                                    for batch_size in config['batch_size']:
                                        ...
                                        for batch_norm_momentum in config['bn_momentum']:
                                            ...
                                            # create dataset
                                            train_dataset = AdniVolumeDataset(splits["train"], input_dim=config['dim'], masking=mask, augment=config['augment_train_set'], heterog_exp=heterogeneous_exp, normalize_non_image_data=config['normalize_non_image_data'])
                                            eval_dataset = AdniVolumeDataset(splits["valid"], input_dim=config['dim'], masking=mask, augment=config['augment_val_set'], heterog_exp=heterogeneous_exp, normalize_non_image_data=config['normalize_non_image_data'])
                                            
                                            # create dataloader
                                            train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=config['shuffle_train_set'], drop_last=True, num_workers=6)
                                            eval_loader = DataLoader(eval_dataset, batch_size=batch_size, shuffle=config['shuffle_val_set'], num_workers=6)
                                            ...
                                            model_dir = datetime.datetime.now().strftime('%Y-%m-%d_%H-%M') \
                                                ...
                                                + ('_normalized' if config['normalize_non_image_data'] else '') \
                                                + f'_loc{location}'  \
                                                + f'_activation_{activation}' \
                                            ...
                                            hparms = {'fixed': config['block_grads'], 'normalized': config['normalize_non_image_data']}
