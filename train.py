                        ...
                        for weight_decay in config['weight_decay']:
                            ...
                            for batch_size in config['batch_size']:
                                ...
                                for batch_norm_momentum in config['bn_momentum']:
                                    ...
                                    # create dataset
                                    train_dataset = AdniVolumeDataset(splits["train"], input_dim=config['dim'], masking=mask, augment=config['augment_train_set'], heterog_exp=heterogeneous_exp, normalize_non_image_data=normalize, is_multiclass_exp=(config['model_args']['n_outputs'] > 1))
                                    eval_dataset = AdniVolumeDataset(splits["valid"], input_dim=config['dim'], masking=mask, augment=config['augment_val_set'], heterog_exp=heterogeneous_exp, normalize_non_image_data=normalize, is_multiclass_exp=(config['model_args']['n_outputs'] > 1))
